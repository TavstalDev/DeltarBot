...
public class GuildSeasonCommand : SimpleCommand
{
    public GuildSeasonCommand(DiscordSocketClient client, WynnHttpClient wynnClient, GuildService guildService) : base(
        "gseason",
        "Gets seasonal information about the provided guild.",
    ...
    public override async Task HandleAsync(SocketSlashCommand data)
    {
        ...
        try
        {
            ...
            {
                description += $"Season {season.Key} • Rating: {season.Value.Rating} • Territories: {season.Value.FinalTerritories} \n";
            }
